...
if "점수" not in data.columns:
    ...
    # 사용자가 입력한 기업명
    input_company = input("비교하고 싶은 기업명을 입력하세요: ")

    # 해당 기업 데이터 필터링
    input_company_data = data[data['기업명'] == input_company]
    ...
    def plot_radar_chart():
        ...
        # 입력받은 기업에 대해 그래프 추가
        if not input_company_data.empty:
            categories = input_company_data['항목'].tolist()
            values = input_company_data['점수'].tolist()
            recent_values = input_company_data['최근값'].tolist()
        ...
        # 레이아웃 설정
        fig.update_layout(
            title=f"{input_company} 기업과 Average 기업 방사형 차트",
            polar=dict(
                radialaxis=dict(
                    visible=True,
